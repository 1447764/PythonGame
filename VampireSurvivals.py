...
# --- [개선] 스킬 업그레이드까지 데이터 중심으로 설계 ---
LEVEL_DATA = [50, 75, 110, 150, 220, 300, 450, 600, 800, 1000, 1250, 1500, 1800, 2200, 2600, 3000]
UPGRADE_DATA = {
    # 패시브 업그레이드
    'WEAPON_DAMAGE':   {'name': '마법탄 공격력 +5',        'type': 'passive', 'target': 'weapon_damage',    'operation': 'add', 'value': 5},
    'WEAPON_COOLDOWN': {'name': '마법탄 연사 속도 +10%',   'type': 'passive', 'target': 'weapon_cooldown',  'operation': 'multiply', 'value': 0.9},
    ...
    'PROJECTILE_SPEED':{'name': '마법탄 속도 +1',         'type': 'passive', 'target': 'projectile_speed', 'operation': 'add', 'value': 1},
    
    # 스킬 획득
    'ACQUIRE_BIBLE': {'name': '성스러운 책 획득', 'type': 'acquire', 'skill_key': 'bible'},
    
    # 스킬 강화: 업그레이드 효과를 데이터로 완벽하게 정의
    'BIBLE_DAMAGE': {
        'name': '성스러운 책 공격력 +8', 'type': 'upgrade', 'skill_key': 'bible',
        'target': 'damage', 'operation': 'add', 'value': 8
    },
    'BIBLE_COUNT':  {
        'name': '성스러운 책 개수 +1', 'type': 'upgrade', 'skill_key': 'bible',
        'target': 'count', 'operation': 'add', 'value': 1,
        'side_effect': 'recreate_sprites' # 개수가 바뀌면 스프라이트를 다시 만들어야 한다는 정보 추가
    },
    'BIBLE_SPEED':  {
        'name': '성스러운 책 회전 속도 +20%', 'type': 'upgrade', 'skill_key': 'bible',
        'target': 'rotation_speed', 'operation': 'multiply', 'value': 1.2
    },
}
...
class Quadtree:
    ...
    def get_index(self, rect):
        ...
        top_quad = (rect.bottom < cy)
        bottom_quad = (rect.top > cy)
        if rect.right < cx:
            if top_quad: index = 1
    ...
    def insert(self, obj):
        ...
        if len(self.objects) > self.max_objects and self.level < self.max_level:
            if self.nodes[0] is None: self.split()
            i = 0
...
class Skill:
    ...
    # [개선] 데이터 기반의 범용 레벨업 메소드. 모든 스킬이 이 메소드를 상속받아 사용.
    def level_up(self, upgrade_key):
        self.level += 1
        data = UPGRADE_DATA.get(upgrade_key)
        if not data: return

        # target, operation, value가 데이터에 정의되어 있으면 능력치를 변경
        if all(k in data for k in ('target', 'operation', 'value')):
            ...
            if op == 'add':
                setattr(self, target_attr, current_val + val)
            elif op == 'multiply':
                setattr(self, target_attr, current_val * val)

        # side_effect (부가 효과)가 데이터에 정의되어 있으면 해당 이름의 메소드를 실행
...
class BibleSkill(Skill):
    ...
    # [개선] level_up 메소드를 직접 구현할 필요 없음. 부모 클래스(Skill)의 범용 메소드를 상속받아 사용.
    # 이렇게 하면 if/elif 문이 사라지고 코드가 매우 깔끔해짐.
...
SKILL_CLASSES = {'bible': BibleSkill}
...
class Player(pygame.sprite.Sprite):
    ...
    def reset(self):
        ...
        self.hp = self.max_hp
        self.weapon_cooldown, self.weapon_damage, self.projectile_speed = 500, 10, 10
        self.level, self.exp, self.exp_to_next_level = 1, 0, LEVEL_DATA[0]
        self.invincible, self.last_shot_time, self.invincible_duration, self.last_hit_time = False, 0, 1000, 0
    ...
    # --- [개선] 제곱근 연산을 피하기 위해 distance_squared_to 사용 ---
    def find_closest_enemy(self):
        ...
        for enemy in self.game.enemies:
            ...
            if dist_sq < closest_dist_sq:
                closest_dist_sq, closest_enemy = dist_sq, enemy
...
class Enemy(pygame.sprite.Sprite):
    def __init__(self, game):
        ...
        self.exp_drop, self.damage = 15, 5
        self.skill_hit_cooldown, self.last_skill_hit_time = 500, 0
    ...
    def update(self):
        ...
        nearby_enemies = self.game.quadtree.retrieve([], search_rect)
        ...
        if final_vec.length() > 0:
            self.pos += final_vec.normalize() * self.speed
        self.rect.center = self.pos
...
class Projectile(pygame.sprite.Sprite):
    def __init__(self, game, pos, target_enemy):
        ...
        self.speed, self.damage = self.game.player.projectile_speed, self.game.player.weapon_damage
        self.lifespan, self.spawn_time = 2000, pygame.time.get_ticks()
        self.vel = (target_enemy.pos - self.pos).normalize() * self.speed
...
class Game:
    ...
    def load_game_data(self):
        try:
            ...
            self.gold, self.permanent_upgrades = 0, {}
    ...
    def new_game(self):
        ...
        self.enemy_pool = []
        self.quadtree = Quadtree(0, (0, 0, WORLD_WIDTH, WORLD_HEIGHT)) # 쿼드트리 생성
        self.spawn_timer, self.spawn_interval = 0, 500
        self.game_state, self.kill_count = 'PLAYING', 0
    ...
    def manage_enemy_spawning(self):
        ...
        if self.spawn_timer > self.spawn_interval:
            ...
            if len(self.enemies) < MAX_ENEMIES_ON_SCREEN:
                ...
                pos = (max(0, min(x, WORLD_WIDTH)), max(0, min(y, WORLD_HEIGHT)))
                ...
                if event.type == pygame.MOUSEBUTTONDOWN and event.button == 1:
                    for i, rect in enumerate(self.upgrade_option_rects):
                        if rect.collidepoint(event.pos):
                            self.apply_upgrade(self.current_upgrade_options[i]); break
    ...
    def update(self):
        if self.game_state == 'PLAYING':
            ...
            for gem in pygame.sprite.spritecollide(self.player, self.exp_gems, True):
            ...
            for enemy, skills in skill_hits.items():
                ...
                if now - enemy.last_skill_hit_time > enemy.skill_hit_cooldown:
                    ...
                    skill_sprite = skills[0]
                    if isinstance(skill_sprite, BibleSprite):
                        if gem_pos := enemy.take_damage(skill_sprite.skill.damage):
                            self.kill_count += 1; gem = ExpGem(gem_pos, enemy.exp_drop)
                            self.all_sprites.add(gem); self.exp_gems.add(gem)
    def draw(self):
    ...
    def draw_start_menu(self):
        ...
        self.screen.blit(gold_text, gold_text.get_rect(center=(SCREEN_WIDTH/2, SCREEN_HEIGHT - 50)))
    def draw_shop_screen(self):
        self.screen.fill(DARK_GREY)
        title = self.header_font.render("상점", True, WHITE)
    ...
    def draw_credits_screen(self):
        self.screen.fill(DARK_GREY)
        title = self.header_font.render("제작진", True, WHITE)
        self.screen.blit(title, title.get_rect(center=(SCREEN_WIDTH/2, 150)))
    ...
    def draw_game_ui(self):
        ...
        elapsed_ticks = (self.paused_time if self.game_state != 'PLAYING' else pygame.time.get_ticks()) - self.game_start_time
        ...
        kills = self.ui_font.render(f"처치: {self.kill_count}", True, WHITE)
        ...
        if self.game_state == 'LEVEL_UP':
            ...
            title = self.header_font.render("LEVEL UP!", True, YELLOW)
            ...
            for i, key in enumerate(self.current_upgrade_options):
                ...
                rect = pygame.Rect(SCREEN_WIDTH/2 - opt_w/2, start_y + i * (opt_h + 20), opt_w, opt_h)
            ...
            self.screen.blit(overlay, (0, 0))
            title = self.header_font.render("GAME OVER", True, RED)
